 ...
 * @LastEditTime: 2019-09-25 10:07:06
 * @LastEditors: Please set LastEditors
 -->
<template>
  <div class="sun-career-picker">
    <sun-cell :title="title" :value="career" :border="border" center clickable arrowDirection="right"
      @click="clickCell">
      <slot name="extra" v-if="placeholder && !career"><span
          class="sun-career-picker-placeholder">{{placeholder}}</span></slot>
    </sun-cell>
    ...
    <sun-popup v-model="show" :zIndex="zIndex" position="bottom" :get-container="getContainer"
      ...
      <div class="sun-career-picker-toolbar sun-line-top-bottom">
        ...
        <div class="sun-career-picker-title sun-ellipsis" v-if="keyConfig[activeIndex]"
          v-text="keyConfig[activeIndex].name" />
        <div class="sun-career-picker-confirm"
          :class="{'sun-career-picker-disabled': cacheValues.length !== keyConfig.length}" @click="confirmPicker">确定
        </div>
      </div>
      <div class="sun-career-picker-steps sun-line-top-bottom">
        <div class="sun-career-picker-step" v-if="index <= activeIndex" v-for="(item, index) in keyConfig"
          :key="item.name">
          ...
          <div class="sun-career-picker-step-circle" :class="{'sun-career-picker-step-finish': index < activeIndex}">
          </div>
          <div class="sun-career-picker-step-line" v-if="index < activeIndex"></div>
        </div>
      </div>
      <div class="sun-career-picker-list">
        <div class="sun-career-picker-list-item" v-for="item in pickerData" :key="item.code"
          :class="{'sun-career-picker-list-active': item.code === pickerValue.code}" @click="changePicker(item)">
          <span class="sun-ellipsis">{{item.text}}</span>
          <img class="sun-career-picker-list-icon" v-if="item.code === pickerValue.code"
