 ...
 * @LastEditTime: 2019-09-25 09:59:53
 * @LastEditors: Please set LastEditors
 -->
<template>
  <div class="sun-swipe">
    <div :style="trackStyle" class="sun-swipe-track" @touchstart="onTouchStart" @touchmove="onTouchMove"
      @touchend="onTouchEnd" @touchcancel="onTouchEnd" @transitionend="$emit('change', activeIndicator)">
      <slot />
    </div>
    <div v-if="showIndicators && count > 1" :class="['sun-swipe-indicators', { 'sun-swipe-vertical': vertical }]">
      <i v-for="index in count" :key="index"
        :class="['sun-swipe-indicator', { 'sun-swipe-vertical': vertical, 'sun-swipe-active': index - 1 === activeIndicator }]" />
    </div>
  </div>
</template>
